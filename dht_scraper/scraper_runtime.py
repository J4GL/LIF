...
def settings_error(port: int, nodes: int, web_host: str, web_port: int, duration: Optional[float], fetch_workers: int, batch_size: int, interval: float) -> Optional[str]:
    ...
    elif not 1 <= fetch_workers <= MAX_CONNECTIONS_LIMIT:
        result = "--fetch-workers must be between 1 and %d" % MAX_CONNECTIONS_LIMIT
    elif batch_size < 1:
...
class RuntimeSettings:
    ...
    def __init__(
        ...
        fetch_workers: int = DEFAULT_MAX_CONNECTIONS,
...
class ScraperRuntime:
    ...
    # Parents: start
    # Keywords: sockets, bind, nodes, ipv6 next to ipv4, same ids
    def open_sockets(self) -> None:
        ...
        if self.settings.ipv6_enabled:
            try:
                self.node_sockets += self.socket_factory(self.settings.port, self.settings.nodes, socket.AF_INET6, [node.own_id for node in self.node_sockets])
            except OSError as error:
                LOGGER.warning("IPv6 DHT disabled: %s", error)
        assert len(self.node_sockets) in (self.settings.nodes, 2 * self.settings.nodes)
    ...
    def start_fetching(self) -> None:
        assert self.fetch_engine is None
        if not self.settings.fetch_enabled:
            LOGGER.info("metadata fetching disabled")
            return
        raise_open_file_limit(self.settings.fetch_workers + FILE_LIMIT_MARGIN)
        self.fetch_engine = FetchEngine(self.catalog, self.stop_event, self.fetch_function, self.settings.fetch_workers, retry_function=self.retry_function, utp_function=self.utp_function)
        self.fetch_engine.start()
        assert self.fetch_engine.thread is not None
    ...
    def log_summary(self) -> None:
        ...
        LOGGER.info(
            "summary: nodes=%d sent=%d received=%d samples=%d hashes=%d discovered=%d metadata=%d attempts=%d connections=%d in_progress=%d failed=%d crawl_queue=%d lookups=%d",
            stats["nodes"], stats.get("packets_sent", 0), stats.get("packets_received", 0), stats.get("samples_received", 0),
            stats["hashes_seen"], stats["hashes_discovered"], stats["with_metadata"], stats.get("fetch_attempts", 0), stats.get("fetch_connections", 0),
            stats["fetch_in_progress"], stats["fetch_failed"], stats.get("queue_size", 0), stats.get("active_lookups", 0),
        )
